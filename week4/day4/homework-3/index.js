...
class ShoppingCart {
  ...
  addItem(productId, quantity = 1) {
    try {
      ...
      product.stock -= quantity; // Sepete eklenen ürün stoktan düşülüyor
      this.calculateTotal();
      this.updateUI();
    } catch (error) {
  ...
  removeItem(productId) {
    try {
      ...
      if (product) {
        product.stock += item.quantity; // Stok güncellemesi
      }
  ...
  calculateTotal() {
    this.total = this.items.reduce((sum, item) => {
      return sum + item.price * item.quantity; // Miktar çarpımı eklendi
    }, 0);

    if (this.discountApplied && this.total > 0) {
      this.total *= 0.9; // İndirim %10 uygulanıyor (0.1 değil 0.9 ile çarpılmalı)
    }
  }
        ...
        .map(
          (item) => `
                <div class="cart-item">
                    ...
                    <button onclick="app.removeItem(${item.productId})">Sil</button>
                </div>
            `
        )
        .join("");
